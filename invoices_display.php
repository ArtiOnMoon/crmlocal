...
$invoice_bank=clean($_POST['invoice_bank']);
$cond='';
if ($invoice_customer!='' AND $invoice_customer!='1')$cond.=' AND invoice_customer="'.$invoice_customer.'"';
if ($invoice_type!='' AND $invoice_type!='0')$cond.=' AND invoice_type="'.$invoice_type.'"';
...
if ($invoice_bank!='' AND $invoice_bank!='0')$cond.=' AND invoice_our_bank_det="'.$invoice_bank.'"';
//END CONDITIONS
...
$db =  db_connect();
$query2='SELECT invoice_id FROM invoices WHERE invoice_type=2 AND invoice_deleted=0'.$cond.$search;
...
$query= 'SELECT invoices.*,customers.cust_id,customers.cust_short_name,invoices_statuses.inv_stat_name, our_companies.our_name,curr_name, our_details.name as bank, '
        ...
        . 'LEFT JOIN our_details ON invoice_our_bank_det=our_details.id '
        . 'LEFT JOIN currency ON curr_id=invoice_currency '
        . 'WHERE invoice_type=2 AND invoice_deleted=0';
$query.=$cond.$search.$sort.' LIMIT '.$limit.' OFFSET '.$offset;
if(!$result=$db->query($query))exit($query);
...
<div id="main_subbody">
    <div id="table_wrap">
        ...
        while($row = $result->fetch_assoc()){
            ...
            echo '<tr>'
            . '<td>',$type,'</td>'
            , '<td class="num">', view_invoice_by_id($row['invoice_id'],$row['invoice_num']),'</td>'
            //, '<td>',invoice_type($row['invoice_type']),'</td>'
