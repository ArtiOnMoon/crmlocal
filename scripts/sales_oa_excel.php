...
//Открытие Шаблона
$document=IOFactory::load($_SERVER['DOCUMENT_ROOT'].'/templates/invoice_co.xlsx');
...
//Реквизиты и пр
$document->getActiveSheet()
    ->setCellValue('D2', $sale['sales_invoice'])
...
if ($sale['sales_delevery_terms']!=='') $document->getActiveSheet()
    ->setCellValue('B30', 'Delivery terms:')
...
$row=26;
$i=1;
$entries=$db->query('SELECT * FROM sales_content WHERE scont_sale_id="'.$id.'" and scont_cfm_qty>0');
...
header('Content-Disposition: attachment;filename="OA_'.$sale['id'].'.'.$sale['sales_no'].$sale['client_of'].'.xlsx"');
