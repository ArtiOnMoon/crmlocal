...
//END SORT
//Выбираем головы комплектов
 $query='SELECT stock_new.*, stock_nmnc.*,mnf_short_name, stock_stat_name,'
        . 'stockl_name, stock_cat_name, curr_name, our_companies.id, our_companies.our_name,sales_id, 0 as level '
        . 'FROM stock_new '
        . 'LEFT JOIN stock_nmnc ON stock_nmnc_id=stnmc_id '
        ...
        . 'WHERE stock_new.stock_deleted=0 AND stock_is_compl=1 AND stock_compl_id IS NULL';
...
while($complect = $complects->fetch_assoc()){
    $global_array[]=$complect;
}
//Подзапрос поиска содержимого
    $subquery='SELECT stock_new.*, stock_nmnc.*,mnf_short_name, stock_stat_name,'
...
while($stop!=1){
    $temp_array=array();
        ...
        $key++;
        if ($key>=count($global_array))break;
}
?>
...
<div id="main_subbody">
    ...
    <thead onclick="table_sort(event,'stock')">
        <th width="50" colspan="6" keyword="stock_id" <?php sort_class('stock_id',$sort_field,$sort_type);?>>ID</th>
    ...
    <tbody>
<?php
if (count($global_array)===0 ) {exit('<tr><td colspan="18">No results</td></tr></table>');}
//Complects - список комплектов
foreach ($global_array as $complect){
    echo '<tr class="',stock_tr_is_sold($complect['stock_status']);
    if ($complect['level']==0) echo ' stock_complects_row';
    echo '">';
    echo stock_view_complect_link($complect['stock_id'],$complect['level'])
        ...
        //, '<td>',$row['curr_name'],'</td>'
        , '<td>',stock_view_po($complect['stock_po_comp'],$complect['stock_po'],$complect['stock_po_type']),'</td>'
        , '<td>',stock_view_sales($complect['stock_so_comp'],$complect['stock_so'],$complect['stock_so_type']).'</td>'
        , '</tr>';
}
?>
    </tbody></table></form>
